...
WHERE (UserLinks.IndexTable = @tableName) 
  ...
  AND ((Trackables.RecordVersion < 0 AND Trackables.CheckedOutUserId = @userId)
   OR ((Trackables.CheckedOut = 0 AND Trackables.RecordVersion = (SELECT TOP 1 Trackables.RecordVersion FROM Trackables WHERE (Attachments.TrackablesId = Trackables.Id) ORDER BY Trackables.RecordVersion DESC))))
ORDER BY UserLinks.AttachmentNumber, ABS(Trackables.RecordVersion), Attachments.PageNumber ASC, Attachments.PointerId