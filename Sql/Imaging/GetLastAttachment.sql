...
WHERE (UserLinks.IndexTable = @tableName) 
  ...
  AND (ISNULL(UserLinks.AttachmentNumber, 1) = (SELECT TOP 1 ISNULL(UserLinks.AttachmentNumber, 1) FROM UserLinks WHERE (UserLinks.IndexTable = @tableName) AND (UserLinks.IndexTableId = @tableId) ORDER BY UserLinks.AttachmentNumber DESC))
